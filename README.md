...
## 2. 开启项目开发
    ...
    使用脚手架创建项目: vue-cli3
    安装所有依赖/指定依赖(下载安装依赖最好统一用npm或者yarn，不要混用（因为yarn与npm两个工具包
    处理不是一样的，混用容易出现丢包的情况），混用容易出现问题；
    如果项目的依赖之前是用yarn下载的，而你现在想用npm来下载，就需要删除目前的node_modules与yarn.lock;
    然后用npm install重新下载所有依赖，那么后面就可以一直用npm来下载依赖了)
    开发环境运行
    生产环境打包与运行
...
### 6). 解决ajax的跨越域问题
    ...
    注意：只要做前后端分离（前台应用，后台应用分别运行）的项目，开发环境必然存在ajax跨域的问题
